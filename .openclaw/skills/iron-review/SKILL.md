---
name: iron-review
description: "Code review on 2 axes -- Standards (quality + security) and Spec (requirements + tests)."
---

# /iron:review

Code review on two independent axes: **Standards** (is the code well-built?) and
**Spec** (does it build the right thing?). A change can pass one and fail the
other -- reporting them separately prevents masking.
...
1. **CLAUDE.md / CONTRIBUTING.md / CODING_STANDARDS.md** -- repo standards override Fowler smells.
2. **ironworks/specs/** -- current requirements (if exists)
3. **ironworks/changes/** -- related in-flight changes
4. **The full files being changed**
...
12 smells from Refactoring ch. 3, used as heuristics. Repo standards override.
...
4. **Platform feature?** -- ignoring a framework feature
5. **Already-installed dep?** -- new dep for what an existing one handles
...
Only flag what you can see in the diff.

##### Over-Engineering Tags

Tag each Standards finding: `delete:` (dead code) | `stdlib:` (use standard library) | `native:` (use platform feature) | `yagni:` (unnecessary abstraction) | `shrink:` (can be shorter).
...
- Does the code do anything out of scope (scope creep)?
...
- Tests must test at pre-agreed seams -- test the contract, not the wiring.
...
- **FIX AND RESHIP:** Has blockers. List them.
...
Over-engineering-only scan. Skip Spec (Axis 2). Focus on the discipline ladder and Fowler smells related to unnecessary complexity.
...
- **Be specific.** "Replace this 40-line validator with `pydantic.field_validator`" beats "this could be simpler."
- **Name the line.** Every finding references file:line.
- **Tag the finding.** Every Standards finding gets a tag when one applies.
- **Don't nitpick style.** Trust the linter.
- **Praise good work.** Clean and tested? Say "SHIP IT ✓" and mean it.
...
- **Repo standards override smells.** Repo standard wins over a Fowler smell.
