# Ironworks for OpenAI Codex

> Use Ironworks skills with Codex. 12 skills · 7 layers · 60+ commands.

**Part of [Ironworks](https://github.com/RahulHulsure/-Ironworks)** — Industrial-grade development pipeline for AI coding agents.

## Install

### Quick (copy files)

```bash
# From the ironworks repo root:
cp platforms/codex/AGENTS.md your-project/AGENTS.md
```

### Via installer

```bash
./install.sh --platform codex
# or on Windows:
.\install.ps1 -Platform codex
```

## What's Included

| File | Purpose |
|------|---------|
| `AGENTS.md` | All Ironworks skills as a single Codex agent instructions file |

## How It Works

Codex reads `AGENTS.md` at the project root and uses it as the agent's system-level instructions, shaping how it plans, builds, and reviews code.

## Available Commands

After installing, use these in Codex chat:

```
/iron:help                    # See all commands
/iron:init                    # Bootstrap a project
/iron:graph                   # Map codebase dependencies
/iron:spec propose <feature>  # Plan before building
/iron:review                  # Smart code review
```

→ [Full command list](https://github.com/RahulHulsure/-Ironworks#-all-12-skills) | [All platforms](https://github.com/RahulHulsure/-Ironworks#-supported-platforms)
